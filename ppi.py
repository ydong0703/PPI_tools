...
# customized function -- output estimate and variance
def ppi_mean_var(
    Y,
    Yhat,
    Yhat_unlabeled,
    ...
    return {'est':ppi_pointest, 'sd': np.sqrt(imputed_std**2 + rectifier_std**2)}
...
# customized function -- output estimate and sd
def ppi_ols_var(
    X,
    Y,
    Yhat,
    ...
    return {"est": ppi_pointest, "sd":np.sqrt(np.diag(Sigma_hat) / n)}
...
def ppi_logistic_pointestimate(
    ...
    def rectified_logistic_grad(_theta):
        return (
            ...
            * X_unlabeled.T
            @ (
                w_unlabeled.reshape(-1,1)
                * (safe_expit(X_unlabeled @ _theta).reshape(-1,1) - Yhat_unlabeled)
            )
            - lam_curr / n * X.T @ (w.reshape(-1,1) * (safe_expit(X @ _theta).reshape(-1,1) - Yhat))
            + 1 / n * X.T @ (w.reshape(-1,1) * (safe_expit(X @ _theta).reshape(-1,1) - Y))
        )
